...
class HarmonicVoltageSource:
    ...
    def __init__(self, elmvac, app_or_dyn_sim_interface, composite_frame=None):
        """Initialization.
        Arguments:
        ...
        self.elmvac = self.pfdi._handle_single_pf_object_or_path_input(elmvac)
        if composite_frame:
    ...
    def set_phase(self, phase: str, params: dict):
        ...
        for param, value in params.items():
            ...
            self.pfdi.set_dsl_obj_array(
                phase_dsl, value, array_num=array_num, size_included_in_array=False
            )

    def set_symmetric(
        self, params: dict, same_angular_shift_for_harmonics_of_all_phases=False
    ):
        """
        ...
        for phase in ("a", "b", "c"):
            ...
            for param, time_value_pairs in params.items():
                ...
                if "phi" in param:
                    if (
                        same_angular_shift_for_harmonics_of_all_phases
                        and not param == "phi1"
                    ):
                        additional_angle = 0
                    else:
                    ...
                    for idx, angle_at_time in enumerate(time_value_pairs):
                        time_value_pairs_copy[idx][1] = (
                            angle_at_time[1] + additional_angle
                        )
                self.pfdi.set_dsl_obj_array(
                    dsl_model_of_phase,
    ...
    def reset_symmetric_and_only_fundamental_frequency(
        self, fundamental_frequency=50, magnitude=1, angle=0
    ):
        """
    ...
    def set_nominal_voltage_of_dsl_model(self, voltage_kV):
        self.pfdi.get_single_obj(
            "per_unit_to_nominal", parent_folder=self.comp_frame
        ).SetAttribute("nominal_value", voltage_kV)
    ...
    def get_dsl_model_of_phase(self, phase: str):
        return self.pfdi.get_single_obj("phase_" + phase, parent_folder=self.comp_frame)
